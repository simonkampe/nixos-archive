...

{
  ...
  programs.git = {
    ...
    aliases = {
      ...
      template = "!f() { temp_dir=$(mktemp -d); wget -c https://github.com/simonkampe/template-\"$1\"/archive/main.tar.gz -O - | tar -xz -C $temp_dir ; mkdir -p \"$2\" ; mv $temp_dir/template-\"$1\"-main/* \"$2\";  mv $temp_dir/template-\"$1\"-main/.* \"$2\";}; f";
    };
